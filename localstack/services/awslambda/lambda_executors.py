        ...
        with self.docker_container_lock:
            ...
            if status < 1:
                ...
                cmd = (
                    ...
                    '  %s'  # docker options
                    ' lambci/lambda:%s'
                ) % (container_name, env_vars_str, network_str, docker_options, runtime)
                LOG.debug(cmd)
...
class LambdaExecutorSeparateContainers(LambdaExecutorContainers):
    ...
    def prepare_execution(self, func_arn, env_vars, runtime, command, handler, lambda_cwd):
        ...
        if config.LAMBDA_REMOTE_DOCKER:
            ...
            ) % (entrypoint, env_vars_string, network_str, docker_options, runtime, command, lambda_cwd)
        else:
            ...
            cmd = (
                ...
                ' --rm'
                ' "lambci/lambda:%s" %s'
            ) % (entrypoint, lambda_cwd_on_host, env_vars_string, network_str, docker_options, runtime, command)
        return cmd
